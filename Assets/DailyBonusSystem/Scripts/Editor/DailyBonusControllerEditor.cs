...
public class DailyBonusControllerEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		for (int i = 0; i < main.BonusItems.Count; i++) {
			...
			main.BonusItems [i].name = EditorGUILayout.TextField (main.BonusItems [i].name);
			EditorGUILayout.EndHorizontal ();

			EditorGUILayout.BeginHorizontal ();
			EditorGUILayout.LabelField("Probability (" + (100f * main.BonusItems[i].ProbabilityPercent / total_probability).ToString("F2") + "%)", GUILayout.Width(130));
			main.BonusItems[i].ProbabilityPercent = Mathf.RoundToInt(EditorGUILayout.Slider(main.BonusItems[i].ProbabilityPercent, 1, 1000));
			EditorGUILayout.EndHorizontal();

			EditorGUILayout.BeginHorizontal ();
			EditorGUILayout.LabelField ("Angle",GUILayout.Width(100));
			main.BonusItems[i].Angle = Mathf.RoundToInt(EditorGUILayout.Slider(main.BonusItems[i].Angle, 0, 360));
			EditorGUILayout.EndHorizontal();
